...
function ofertaPagada(req, res) {
  ...
  console.log("el ide" + params);
  NuevaOferta.updateMany({ _id: params }, { '$set': { estadoPago: "2", AdministradorPago: req.user.sub, fechaAceptado:fecha } }, (err, ofertaPagada) => {
...
function getOfertasPagadasFecha(req, res)
{
  var userId = req.user.sub;
  console.log(userId);
   var fecha= req.params.fecha;
   console.log("fecha que viene"+fecha);

  var message = NuevaOferta.find({'$and':[{ estadoPago: 2 },{fechaAceptado:fecha}]}).populate({ path: 'emitter' }).populate({ path: 'contratista' }).exec((err, messagess) => {
    if (err) {
      return res.status(500).send({
        message: 'No se ha podido obtener las ultimas ofertas'
      });
